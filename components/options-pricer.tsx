...
export default function OptionsPricer() {
  ...
  const priceOptions = () => {
    const newPricings = pricingRows.map((row) => {
      ...
      const premium = Math.floor(value * 10000000)
      const delta = row.type === "call" ? Math.floor(Math.random() * 500000) : -Math.floor(Math.random() * 500000)
      const gamma = Math.floor(Math.random() * 50000)
      const vega = Math.floor(Math.random() * 25000)
      const theta = -Math.floor(Math.random() * 8000)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">FX Options Pricer</h2>
            ...
            <TableBody>
              {recentPricings.map((option) => (
                <TableRow key={option.id}>
                  ...
                  <TableCell className="text-right">${option.premium.toLocaleString()}</TableCell>
                  <TableCell className={`text-right ${option.delta >= 0 ? "text-emerald-500" : "text-red-500"}`}>
                    {option.delta >= 0 ? "+" : ""}
                    {formatCurrency(option.delta)}
                  </TableCell>
                  <TableCell className="text-right">{formatCurrency(option.gamma)}</TableCell>
                  <TableCell className="text-right">{formatCurrency(option.vega)}</TableCell>
                  <TableCell className={`text-right ${option.theta >= 0 ? "text-emerald-500" : "text-red-500"}`}>
                    {option.theta >= 0 ? "+" : ""}
                    {formatCurrency(option.theta)}
                  </TableCell>
